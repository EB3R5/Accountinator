function organizeAndAddRowsToProjectionByCategory() {
  ...
  for (var i = 1; i < data.length; i++) { // Skip the header row (assumes the first row is a header)
    ...
    var key = spendConfig + "_" + category + "_" + group + "_" + type + "_" + cashflowType + "_" + budget;

    // If the key exists in the object, add to the amount
    if (key in organizedData) {
      organizedData[key][month] += amount;
      organizedData[key][13] += amount; // Add to the "Year Total" column
    } else {
      // Otherwise, initialize the object for the key
      organizedData[key] = [spendConfig, category, group, type, cashflowType, budget];
      for (var j = 0; j < 12; j++) {
        organizedData[key].push(j === month ? amount : '');
      }
