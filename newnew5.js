function organizeAndAddRowsToProjectionByCategory() {
  ...
  for (var i = 1; i < data.length; i++) { // Skip the header row (assumes the first row is a header)
    ...
    var targetRow = (year === new Date().getFullYear() && month === new Date().getMonth()) ? 30 : (month + 7); // 7 is used to offset rows to start from row 8
    ...
    if (key in organizedData) {
      ...
      for (var j = 0; j < 12; j++) {
        organizedData[key].push(j === targetRow - 8 ? amount : '');
      }
