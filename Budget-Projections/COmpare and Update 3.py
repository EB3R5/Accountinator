...
def update_google_sheet_with_new_categories(google_sheet, worksheet_name, json_file_path, credentials_file_path):
    ...
    # Ensure columns are ordered correctly before comparison
    # Adjust the column names as per your JSON structure and Google Sheet
    expected_columns = ['Category', 'Group', 'Type', 'Cashflow Type', 'Budget']
    json_df = json_df[expected_columns]
    ...
    if not new_categories_df.empty:
        for index, row in new_categories_df.iterrows():
            # Convert row to list with explicit column ordering
            clean_row = [row[col] for col in expected_columns]
